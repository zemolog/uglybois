...
async def on_message(message):
    chat_filter = ["shit", "fuck", "ass", "bitch", "fck", "sht", "shiz", "slut", "skank", "dildo", "gringo", "cockmaster", "pussy", "asshole", "faggot", "nigga", "hoe", "N.igger", "n_i_G_G_E_R", "Ni.gger", "nig.ger", "nigg.er", "retard", "nigger", "kys"]
    bypass_filter = []
    ...
    if url and message.channel.id == "493148418361786378":
        await bot.delete_message(message)  
    prefix = any(map(message.content.startswith, ['?', '!', '!']))
    if prefix and message.channel.id == "493148418361786378":
      await bot.delete_message(message)
      muted_bots = ['235088799074484224','485784238277263362', '155149108183695360']
      async for msg in bot.logs_from(bot.get_channel("493148418361786378"), limit=4):
...
def check_configs():
    if settings.bot_settings == settings.default_settings:
        print("flanke bot - First run configuration\n")
...
if __name__ == '__main__':
    ...
    try:
        ...
        print("Invalid login credentials. Restart flanke bot and configure it"
              " properly.")
