...
#
# A module of owner commands. by Cal_9000
# These Commands are reserved for the owner named during setup.
#

log = logging.getLogger("flanke_bot.owner")
...
class Owner:
    ...
    async def _set(self, ctx):
        """Changes Flanke Bot's global settings."""
    ...
    async def _token(self, token):
        """Sets Flanke Bots's login token"""
        if len(token) < 50:
    ...
    async def version(self):
        """Shows Flanke Bot's current version"""
